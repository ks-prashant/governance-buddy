...
3. **Grounded generation + validation** (Steps A/B) for GDPR — hit groundedness/citation targets on that subset. This is the riskiest quality bar; prove it early. **[Not started. Scope note added at build: build the generation endpoint STREAMING from the start (§8.1), not as a blocking call that gets streaming retrofitted in step 5.]** The measured pre-generation latency (§8.1, §11) leaves no slack for a non-streaming first attempt here — treat SSE as part of this step's definition of done, not step 5's.
